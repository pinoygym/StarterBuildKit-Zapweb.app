    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div id="adjustment-slip-content" className="space-y-4 p-6 bg-white text-black">
          ...
          {/* Warehouse Section */}
          <div className="warehouse-section border-2 border-gray-700 bg-blue-50 p-4 rounded-lg text-center">
            <div className="warehouse-label text-xs uppercase text-gray-600 mb-1">Adjusted At</div>
            <div className="warehouse-name text-lg font-bold mb-1">{adjustment.warehouseName}</div>
          </div>
          ...
          <table className="items-table w-full border-collapse my-4">
            ...
            <tbody>
              {adjustment.items.map((item) => {
                const conversionDisplay = item.conversionFactor
                  ? `1 ${item.baseUOM} = ${item.conversionFactor} ${item.actualBaseUOM || item.baseUOM}`
                  : '—';
