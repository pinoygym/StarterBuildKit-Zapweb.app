        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                <strong>Warehouse:</strong> {item.Warehouse.name}
              </p>
              <p className="text-sm text-muted-foreground">
                <strong>Current Quantity:</strong> {currentQuantity} {item.Product.baseUOM}
              </p>
              {difference !== 0 && (
                <p className={`text-sm font-medium ${difference > 0 ? 'text-green-600' : 'text-red-600'}`}>
                  <strong>Adjustment:</strong> {difference > 0 ? '+' : ''}{difference} {item.Product.baseUOM}
                </p>
              )}
            </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>New Quantity ({item.Product.baseUOM})</FormLabel>
                  <FormControl>
                    <NumberInput
                      step={0.01}
                      min={0}
