    ...
    try {
      ...
      if (result.success) {
        ...
        setTransferData({
          ...
          batchNumber: 'N/A', // No batch number for inventory items
          quantity: data.quantity,
          uom: item.Product.baseUOM,
          sourceWarehouse: {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Transfer {item?.Product.name} between warehouses
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                <strong>Batch:</strong> N/A
              </p>
              <p className="text-sm text-muted-foreground">
                <strong>Available:</strong> {availableQuantity} {item.Product.baseUOM}
              </p>
            </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Quantity ({item.Product.baseUOM})</FormLabel>
                  <FormControl>
                    <Input
                      type="number"
                      step="0.01"
