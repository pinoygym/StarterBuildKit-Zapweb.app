        ...
        <div id="receipt-content" className="space-y-4 p-4 bg-white text-black">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="info-row flex justify-between">
              ...
              <span className="info-value font-semibold">{sale.Customer?.name || sale.customerName || 'Walk-in'}</span>
            </div>
            <div className="info-row flex justify-between">
              <span className="info-label text-muted-foreground">Date:</span>
              <span className="info-value">{formatDateTime(sale.createdAt)}</span>
            </div>
            <div className="info-row flex justify-between">
              ...
              <tbody>
                {sale.POSSaleItem.map((item, index) => (
                  <tr key={index} className="border-b">
                    ...
                    <td className="center text-center">{formatQuantity(Number(item.quantity))}</td>
            ...
            {sale.paymentMethod === 'cash' && sale.amountReceived && (
              <>
                ...
                <div className="total-row flex justify-between font-semibold">
                  <span>Change:</span>
                  <span>{formatCurrency(Number(sale.change || 0))}</span>
                </div>
              </>
            )}
          </div>
