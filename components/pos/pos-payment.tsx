...
import { SalesAgentSelector } from './sales-agent-selector';
  ...
  const handleCompleteSale = async () => {
      ...
      if (selectedCustomer.creditLimit) {
        ...
        if (outstandingAmount > creditLimit) {
          toast({
            ...
            description: `Customer credit limit is ${formatCurrency(creditLimit)}. Outstanding amount would be ${formatCurrency(outstandingAmount)}`,
            variant: 'destructive',
          });
          return;
        }
    ...
    try {
      ...
      const saleData = {
        ...
        salesAgentId: selectedAgentId && selectedAgentId !== 'no-agent' ? selectedAgentId : undefined,
        subtotal,
            ...
            <div className="flex items-center gap-2">
              ...
              {transactionDiscountType !== 'none' && (
                <NumberInput
                  ...
                  value={Number(transactionDiscountValue)}
                  onChange={(value) => setTransactionDiscountValue(String(value || ''))}
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <NumberInput
                ...
                step={0.01}
                placeholder="0.00"
                value={amountReceived ? Number(amountReceived) : undefined}
                onChange={(value) => setAmountReceived(String(value || ''))}
                className="text-lg"
              />
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <NumberInput
                ...
                step={0.01}
                placeholder="0.00"
                value={partialPayment ? Number(partialPayment) : undefined}
                onChange={(value) => setPartialPayment(String(value || ''))}
                className="text-lg"
              />
            ...
            {partialPayment && parseFloat(partialPayment) > 0 && (
              <div className="p-4 bg-muted rounded-lg space-y-2">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-medium">{formatCurrency(parseFloat(partialPayment))}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium">Balance to AR</span>
                  <span className="text-lg font-bold text-primary">
                    {formatCurrency(total - parseFloat(partialPayment))}
                  </span>
                </div>
              </div>
            )}
