...
export function APPaymentDialog({ open, onClose, record, onSuccess }: APPaymentDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const response = await fetch('/api/ap/payment', {
        method: 'POST',
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Record payment to {record.supplier?.companyName || 'supplier'}
          </DialogDescription>
        </DialogHeader>
