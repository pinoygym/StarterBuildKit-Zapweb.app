      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-right">Tax Exempt</TableHead>
            ...
            return (
              <TableRow key={record.id} className={overdue ? 'bg-red-50/50' : ''}>
                ...
                <TableCell className="text-right text-xs text-muted-foreground">
                  {Number(record.taxAmount) > 0 ? formatCurrency(Number(record.taxAmount)) : '-'}
                </TableCell>
                <TableCell className="text-right text-xs text-green-600">
                  {Number(record.discountAmount) > 0 ? `-${formatCurrency(Number(record.discountAmount))}` : '-'}
                </TableCell>
                <TableCell className="text-right text-xs text-muted-foreground">
                  {Number(record.otherCharges) !== 0 ? formatCurrency(Number(record.otherCharges)) : '-'}
                </TableCell>
                <TableCell className="text-right text-xs text-muted-foreground">
                  {Number(record.withholdingTax) !== 0 ? formatCurrency(Number(record.withholdingTax)) : '-'}
                </TableCell>
                <TableCell className={`text-right text-xs ${Number(record.salesDiscount) < 0 ? 'text-red-600' : Number(record.salesDiscount) > 0 ? 'text-green-600' : 'text-muted-foreground'}`}>
