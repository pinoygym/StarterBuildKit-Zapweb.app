...
export function DataMaintenanceDialog<T extends ReferenceDataBase>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Invalidate React Query cache based on the reference data type
      const cacheKeyMap: Record<ReferenceDataType, string[]> = {
        'product-categories': ['product-categories'],
        'expense-categories': ['expense-categories'],
        'payment-methods': ['payment-methods'],
        'units-of-measure': ['units-of-measure'],
        'expense-vendors': ['expense-vendors'],
        'sales-agents': ['sales-agents'],
      };

      const keysToInvalidate = cacheKeyMap[config.type];
      if (keysToInvalidate) {
        ...
        return (
          <div key={field.name} className="space-y-2">
            ...
            <NumberInput
              id={field.name}
              value={formData[field.name]}
              onChange={(value) => setFormData({ ...formData, [field.name]: value || 0 })}
              placeholder={field.placeholder}
              required={field.required}
            />
            {errors[field.name] && (
