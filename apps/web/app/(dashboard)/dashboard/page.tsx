  ...
  return (
    <div className="p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mt-6">
        ...
        <KPICard
          title="Total Stock"
          value={formatNumber(kpis?.totalStock || 0)}
        ...
        <KPICard
          title="Gross Profit"
          value={formatCurrency(Number(kpis?.grossProfit || 0))}
          subtitle="This month"
          icon={TrendingUp}
          loading={loading}
        />
        ...
        <KPICard
          title="Net Profit"
          value={formatCurrency(Number(kpis?.netProfit || 0))}
      ...
      <div className="grid gap-4 md:grid-cols-8 mt-6">
        <SystemOverview branchId={selectedBranch?.id} />
      </div>

      <div className="grid gap-4 md:grid-cols-8 mt-6">
        <AgingChart branchId={selectedBranch?.id} />
      </div>

      <div className="grid gap-4 md:grid-cols-8 mt-6">
        <OrderPipeline branchId={selectedBranch?.id} />
        <RecentActivities branchId={selectedBranch?.id} limit={10} />
      </div>
