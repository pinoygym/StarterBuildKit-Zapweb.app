...
export class ProductGenerator {
  static async generate(prisma: PrismaClient, config: ProductConfig) {
    ...
    const waterCategory = await prisma.category.upsert({
      where: { name: 'Water' },
      update: {},
      create: { name: 'Water', description: 'Bottled water products' },
    });

    const carbonatedCategory = await prisma.category.upsert({
      where: { name: 'Carbonated' },
      update: {},
      create: { name: 'Carbonated', description: 'Carbonated soft drinks' },
    });

    const juiceCategory = await prisma.category.upsert({
      where: { name: 'Juice' },
      update: {},
      create: { name: 'Juice', description: 'Fruit juice beverages' },
    });

    const energyCategory = await prisma.category.upsert({
      where: { name: 'Energy Drinks' },
      update: {},
      create: { name: 'Energy Drinks', description: 'Energy and sports drinks' },
    });

    const sportsCategory = await prisma.category.upsert({
      where: { name: 'Sports Drinks' },
      update: {},
      create: { name: 'Sports Drinks', description: 'Isotonic sports beverages' },
    });
    ...
    for (const product of carbonatedProducts.slice(0, config.carbonatedProducts)) {
      const created = await prisma.product.create({
        data: {
          ...
          categoryId: carbonatedCategory.id,
          baseUOM: product.uom,
          basePrice: product.price,
          averageCostPrice: product.cost,
