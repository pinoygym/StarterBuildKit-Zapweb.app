...
export function useCreateReceivingVoucher() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Invalidate related queries - receiving affects inventory, PO, AP, and dashboard
...
// Cancel receiving voucher mutation
export function useCancelReceivingVoucher() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason: string }) => {
      const response = await fetch(`/api/receiving-vouchers/${id}/cancel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to cancel receiving voucher');
      }

      return data;
    },
    onSuccess: (data) => {
      toast.success(data.message || 'Receiving voucher cancelled successfully');
      // Invalidate related queries
      queryClient.invalidateQueries({ queryKey: ['receiving-vouchers'] });
      queryClient.invalidateQueries({ queryKey: ['receiving-voucher'] });
      queryClient.invalidateQueries({ queryKey: ['purchase-orders'] });
      queryClient.invalidateQueries({ queryKey: ['inventory'] });
      queryClient.invalidateQueries({ queryKey: ['ap'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
    ...
    onError: (error: Error) => {
      toast.error(error.message || 'Failed to cancel receiving voucher');
    },
  });
}
