...
test.describe('Settings', () => {
    ...
    test('should handle seed test data', async ({ page }) => {
        ...
        await expect(page.getByText('Test Data Seeded').first()).toBeVisible({ timeout: 15000 });
    });
    ...
    test('should handle delete table data', async ({ page }) => {
        ...
        await expect(page.getByText('Table Data Deleted').first()).toBeVisible({ timeout: 15000 });
    });
    ...
    test('should handle delete all transactions', async ({ page }) => {
        ...
        // Verify success toast
        await expect(page.getByText('Transactions Deleted').first()).toBeVisible({ timeout: 15000 });
    });
    ...
    test('should display and handle Ethel Test Suite', async ({ page }) => {
        // Verify section exists
        await expect(page.getByText('Ethel.8-v.cc Test Suite')).toBeVisible();

        // Find run button
        const runButton = page.getByRole('button', { name: 'Run Ethel Suite' });
        await expect(runButton).toBeVisible();
        await expect(runButton).toBeEnabled();
