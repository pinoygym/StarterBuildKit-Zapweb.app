...
describe('System Health Check (Selenium E2E)', () => {
  ...
  test('2. Product Module: CRUD', async () => {
    ...
    try {
        ...
        const deleteBtn = await productRow.findElement(By.xpath(".//button[.//svg[contains(@class, 'lucide-trash')]] | .//button[.//svg[contains(@class, 'text-destructive')]]"));
  ...
  test('12. Reports: Navigation Check', async () => {
      ...
      await driver.wait(until.elementLocated(By.xpath("//h1[contains(., 'Reports')]")), 10000);
  });

  test('13. POS: Navigation Check', async () => {
      await driver.get(`${BASE_URL}/pos`);
      // POS usually has a different layout, check for product grid
