...
(async function e2eFlowTest() {
    ...
    try {
        ...
        await driver.sleep(2000); // Wait for search results

        // Find the product row
        const productRow = await driver.wait(until.elementLocated(By.xpath(`//tr[contains(., '${productName}')]`)), 10000);
        ...
        const addUomBtn = await driver.wait(until.elementLocated(By.xpath("//button[contains(., 'Add UOM')]")), 5000);
        await addUomBtn.click();

        // Fill UOM details
        console.log('   Filling UOM details...');
        ...
        const updateBtn = await driver.findElement(By.xpath("//button[contains(text(), 'Update')]"));
        await updateBtn.click();

        // Wait for dialog to close
        await driver.wait(until.stalenessOf(await driver.findElement(By.xpath("//h2[contains(text(), 'Edit Product')]"))), 15000);
        console.log('   Alternate UOM added and Product Updated.');
