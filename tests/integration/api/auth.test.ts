...
describe('Auth API', () => {
  ...
  it('valid login returns 200 and me returns 200 with cookie', async () => {
    ...
    if (![200, 201].includes(r.status)) {
      ...
      console.log('Seed failed status:', r.status);
      console.log('Seed failed body:', text);
      fs.writeFileSync('c:/Users/HI/Documents/GitHub/_deve local/_React Apps/test/seed-error.json', JSON.stringify({ status: r.status, body: text }, null, 2));
    }
