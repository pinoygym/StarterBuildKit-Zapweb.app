...
describe('Accounts Receivable API Integration Tests', () => {
  ...
  afterEach(async () => {
    // Clean up all AR records for the test branch
    ...
    if (arIds.length > 0) {
      await prisma.aRPayment.deleteMany({ where: { arId: { in: arIds } } });
      await prisma.accountsReceivable.deleteMany({ where: { id: { in: arIds } } });
    }
    testAR = null;
  });
  ...
  describe('POST /api/ar - Create AR', () => {
    it('should create AR record successfully', async () => {
      ...
      expect(Number(data.data.totalAmount)).toBe(1000);
      expect(Number(data.data.paidAmount)).toBe(0);
      expect(Number(data.data.balance)).toBe(1000);
  ...
  describe('POST /api/ar/payment - Record Payment', () => {
    ...
    it('should record full payment and update status to paid', async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(Number(data.data.paidAmount)).toBe(1000);
      expect(Number(data.data.balance)).toBe(0);
    ...
    it('should record partial payment and update status to partial', async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(Number(data.data.paidAmount)).toBe(500);
      expect(Number(data.data.balance)).toBe(500);
    ...
    it('should handle multiple partial payments', async () => {
      ...
      expect(Number(payments[1].amount)).toBe(400);
      expect(Number(payments[2].amount)).toBe(300);
    });
