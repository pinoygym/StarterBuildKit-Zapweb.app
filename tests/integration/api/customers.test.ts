...
// Import database reset utility for test isolation
const shouldResetDB = process.env.DATABASE_URL?.includes('test');
let resetDatabase: (() => Promise<void>) | null = null;

if (shouldResetDB) {
    import('../../helpers/db-cleanup').then((module) => {
        resetDatabase = module.resetDatabase;
    });
}
...
describe('Customers API Integration Tests', () => {
    ...
    beforeAll(async () => {
        // Reset database for test isolation (only if using test database)
        if (shouldResetDB && resetDatabase) {
            console.log('🔄 Resetting test database for customers tests...');
            await resetDatabase();
        }
    ...
    describe('POST /api/customers', () => {
        ...
        it('should create a new customer with empty email', async () => {
            ...
            // Clean up
            if (data.data.id) {
                await fetch(`${BASE_URL}/api/customers/${data.data.id}`, {
                    method: 'DELETE',
                    headers,
                });
            }
