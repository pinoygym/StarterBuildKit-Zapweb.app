...
describe('Fund Sources API Integration Tests', () => {
    ...
    afterEach(async () => {
        ...
        if (testFundSource) {
            await prisma.fundTransfer.deleteMany({
                where: { OR: [{ fromFundSourceId: testFundSource.id }, { toFundSourceId: testFundSource.id }] }
            });
        ...
        if (testFundSource2) {
            await prisma.fundTransfer.deleteMany({
                where: { OR: [{ fromFundSourceId: testFundSource2.id }, { toFundSourceId: testFundSource2.id }] }
            });
    ...
    afterAll(async () => {
        if (testBranch?.id) {
            ...
            // Cleanup transfers (optional: filter by branch if possible, or just delete all test ones)
            // Ideally we should track transfers created in tests
            await prisma.fundTransfer.deleteMany({});
        ...
        beforeEach(async () => {
            testFundSource = await prisma.fundSource.create({
                data: {
                    id: randomUUID(),
                    name: 'Source Fund',
                    code: `SRC-${randomUUID().substring(0, 8)}`,
                    type: 'CASH_REGISTER',
            ...
            testFundSource2 = await prisma.fundSource.create({
                data: {
                    ...
                    code: `DEST-${randomUUID().substring(0, 8)}`,
                    type: 'BANK_ACCOUNT',
                    openingBalance: 10000,
                    currentBalance: 10000,
                    currency: 'PHP',
    ...
    describe('GET /api/fund-transfers - List Fund Transfers', () => {
        beforeEach(async () => {
            testFundSource = await prisma.fundSource.create({
                data: {
                    ...
                    code: `TLS-${randomUUID().substring(0, 8)}`,
                    type: 'CASH_REGISTER',
                    openingBalance: 50000,
                    currentBalance: 50000,
                    currency: 'PHP',
            ...
            testFundSource2 = await prisma.fundSource.create({
                data: {
                    ...
                    code: `TLD-${randomUUID().substring(0, 8)}`,
                    type: 'BANK_ACCOUNT',
                    openingBalance: 10000,
                    currentBalance: 10000,
                    currency: 'PHP',
            ...
            await prisma.fundTransfer.create({
                data: {
                    id: randomUUID(),
                    transferNumber: `TRF-TEST-${randomUUID().substring(0, 8)}`,
                    fromFundSourceId: testFundSource.id,
                    toFundSourceId: testFundSource2.id,
                    amount: 5000,
                    transferFee: 0,
        ...
        it('should filter by source fund', async () => {
            ...
            expect(response.status).toBe(200);
            expect(data.data.every((t: any) => t.fromFundSourceId === testFundSource.id)).toBe(true);
        });
        ...
        it('should filter by destination fund', async () => {
            ...
            expect(response.status).toBe(200);
            expect(data.data.every((t: any) => t.toFundSourceId === testFundSource2.id)).toBe(true);
        });
    });
