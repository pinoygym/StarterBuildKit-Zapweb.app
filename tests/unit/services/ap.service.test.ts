  ...
  describe('createAP', () => {
    ...
    it('should create AP record with all discount and charge fields', async () => {
      ...
      vi.mocked(apRepository.create).mockResolvedValue({
        ...
        salesDiscount: input.salesDiscount,
        rebates: input.rebates,
        taxExemption: input.taxExemption,
        paidAmount: 0,
        balance: 5000,
        dueDate: input.dueDate,
        status: 'pending',
      ...
      expect(result.salesDiscount).toBe(-150);
      expect(result.rebates).toBe(75);
      expect(result.taxExemption).toBe(25);
    });
  });
