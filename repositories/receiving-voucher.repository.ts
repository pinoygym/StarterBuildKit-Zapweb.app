...
export class ReceivingVoucherRepository {
  ...
  async updateStatus(id: string, status: string): Promise<ReceivingVoucher> {
    return await prisma.receivingVoucher.update({
      where: { id },
      data: { status, updatedAt: new Date() },
    });
  }

  async update(id: string, data: Prisma.ReceivingVoucherUpdateInput): Promise<ReceivingVoucherWithDetails> {
    const updated = await prisma.receivingVoucher.update({
      where: { id },
      data: {
        ...data,
        updatedAt: new Date(),
