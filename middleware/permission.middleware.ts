...
export async function withPermission(
  ...
  try {
    ...
    // Check if user has required permission (case-insensitive comparison)
    const hasPermission = permissions.some(
      (p) => p.resource.toLowerCase() === requiredResource.toLowerCase() && p.action.toLowerCase() === requiredAction.toLowerCase()
    );
