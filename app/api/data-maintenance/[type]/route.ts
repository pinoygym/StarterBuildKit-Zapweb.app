...
export const GET = asyncHandler(async (request: NextRequest, { params }: { params: Promise<{ type: string }> }) {
  try {
...
export const POST = asyncHandler(async (request: NextRequest, { params }: { params: Promise<{ type: string }> }) {
  try {
    const { type } = await params;
