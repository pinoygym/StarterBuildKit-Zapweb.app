...
export const POST = asyncHandler(async (request: NextRequest) {
  try {
    if (process.env.NODE_ENV === 'production') {
      return Response.json({ success: false, message: 'Forbidden in production' }, { status: 403 })
    }
  ...
  if (!role) {
    ...
    if (existing) {
      ...
      return Response.json({ success: true, userId: existing.id, roleId: role.id, message: 'User updated' }, { status: 200 })
    }
    ...
    return Response.json({ success: true, userId: user.id, roleId: role.id }, { status: 201 })
  } catch (error: any) {
    return Response.json({ success: false, message: String(error?.message || 'Seed error') }, { status: 500 })
  }
}