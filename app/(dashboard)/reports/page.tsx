...
import { SalesAgentPerformance } from '@/components/reports/sales-agent-performance';
import { SalesAgentDetailedReport } from '@/components/reports/sales-agent-detailed-report';
      ...
      case 'receiving-variance':
        tableData = {
          ...
          data: rawData.map((row) => [
            row.supplierName,
            row.totalPOs,
            `${formatPercentage(Number(row.averageVariancePercentage))}`,
                  ...
                  <TableBody>
                    {receivingVariance.map((row) => (
                      <TableRow key={row.supplierId}>
                        ...
                        <TableCell className="text-right">{formatPercentage(row.averageVariancePercentage)}</TableCell>
                  ...
                  <TableBody>
                    {salesData.map((item, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-right">{formatPercentage(item.grossMargin)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="pl-4 border-l-2 border-green-500">
                        <p>POS Sales: {formatCurrency(Number(cashFlow.cashInflows.posSales))}</p>
                        <p>AR Payments: {formatCurrency(Number(cashFlow.cashInflows.arPayments))}</p>
                        <p className="font-bold mt-1">Total: {formatCurrency(Number(cashFlow.cashInflows.total))}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Cash Outflows</p>
                      <div className="pl-4 border-l-2 border-red-500">
                        <p>Expenses: {formatCurrency(Number(cashFlow.cashOutflows.expenses))}</p>
                        <p>AP Payments: {formatCurrency(Number(cashFlow.cashOutflows.apPayments))}</p>
                        <p className="font-bold mt-1">Total: {formatCurrency(Number(cashFlow.cashOutflows.total))}</p>
                      </div>
                    </div>
                    <div className="col-span-2 pt-4 border-t">
                      <p className="text-sm text-muted-foreground">Net Cash Flow</p>
                      <p className={`text-3xl font-bold ${Number(cashFlow.netCashFlow) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <TabsContent value="analytics" className="space-y-4">
          {selectedAgentId ? (
            ...
            <SalesAgentPerformance
              ...
              endDate={dateRange.toDate}
              branchId={selectedBranch?.id}
              onViewDetails={setSelectedAgentId}
            />
          )}
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
