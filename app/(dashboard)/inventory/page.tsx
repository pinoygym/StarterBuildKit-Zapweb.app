...
export default function InventoryPage() {
  ...
  const filteredInventory = useMemo(() => {
    ...
    return inventory.filter((item: InventoryWithRelations) =>
  ...
  const totalValue = filteredInventory.reduce(
    (sum: number, item: InventoryWithRelations) => sum + Number(item.quantity) * Number(item.Product.averageCostPrice || 0),
    0
  );

  const totalItems = filteredInventory.length;
  const totalQuantity = filteredInventory.reduce((sum: number, item: InventoryWithRelations) => sum + Number(item.quantity), 0);
