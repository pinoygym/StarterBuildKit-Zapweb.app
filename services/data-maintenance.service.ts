...
export class DataMaintenanceService {
  ...
  async update<T extends ReferenceDataType>(type: T, id: string, data: UpdateInput<T>) {
    ...
    // Include an updatedAt timestamp for the updated record
    const updatePayload = { ...validatedData, updatedAt: new Date() } as any;
    return await dataMaintenanceRepository.update(type, id, updatePayload);
  }
