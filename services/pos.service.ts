...
export class POSService {
  ...
  async processSale(data: CreatePOSSaleInput): Promise<POSSaleWithItems> {
    ...
    const inventoryUpdates: { productId: string; newQuantity: number }[] = [];
    ...
    for (const item of data.items) {
      ...
      const existingUpdate = inventoryUpdates.find(u => u.productId === item.productId);
      if (existingUpdate) {
        existingUpdate.newQuantity -= baseQuantity;
      } else {
        inventoryUpdates.push({
          productId: item.productId,
          newQuantity: totalAvailableStock - baseQuantity
        });
      }
