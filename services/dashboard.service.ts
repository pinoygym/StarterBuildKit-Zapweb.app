...
export class DashboardService {
  async getKPIs(filters?: DashboardFilters): Promise<DashboardKPIs> {
    ...
    for (const product of products) {
      ...
      const productValue = new Decimal(productTotalStock).times(product.averageCostPrice || 0);
    ...
    for (const branch of branches) {
      ...
      for (const warehouse of branch.Warehouse) {
        for (const item of warehouse.Inventory) {
          const itemValue = new Decimal(item.quantity).times(item.Product.averageCostPrice || 0);
