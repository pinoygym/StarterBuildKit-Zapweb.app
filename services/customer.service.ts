...
export class CustomerService {
  async getAllCustomers(filters?: CustomerFilters, pagination?: { skip?: number; limit?: number }): Promise<CustomerWithRelations[]> {
    return await customerRepository.findAll(filters, pagination);
  }

  async getCustomerCount(filters?: CustomerFilters): Promise<number> {
    return await customerRepository.count(filters);
  }
  ...
  async createCustomer(data: CreateCustomerInput, userId?: string): Promise<Customer> {
    // Validate input
    ...
    await auditService.log({
      ...
      details: { companyName: customer.companyName, customerCode: customer.customerCode }
    });

    return customer;
  }

  async updateCustomer(id: string, data: UpdateCustomerInput, userId?: string): Promise<Customer> {
    ...
    const customer = await customerRepository.update(id, { ...validationResult.data, updatedById: userId });
  ...
  async deleteCustomer(id: string, userId?: string): Promise<void> {
    ...
    await auditService.log({
      ...
      resource: 'CUSTOMER',
      resourceId: id,
      details: { companyName: customer.companyName, status: 'inactive' }
    });
  }
