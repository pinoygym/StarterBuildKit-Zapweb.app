...
export class AlertService {
  ...
  async getLowStockAlerts(branchId?: string, warehouseId?: string): Promise<Alert[]> {
    ...
    for (const product of products) {
      const warehouseStockMap = new Map<string, { stock: Prisma.Decimal; warehouse: any }>();
      ...
      for (const batch of product.InventoryBatch) {
        ...
        if (existing) {
          ...
          warehouseStockMap.set(batch.warehouseId, { stock: new Prisma.Decimal(batch.quantity), warehouse: batch.Warehouse });
        }
      }
