    ...
    return await prisma.$transaction(async (tx) => {
      ...
      for (const item of processedItems) {
        if (item.receivedQuantity > 0) {
          ...
          // Determine quantity in base UOM (e.g., bottles) using conversion factor if needed
          const quantityInBaseUOM = await (async () => {
            if (poItem && poItem.uom.toLowerCase() !== product.baseUOM.toLowerCase()) {
              // Find conversion factor for the PO UOM
              const productWithUOMs = await tx.product.findUnique({
                where: { id: item.productId },
                include: { ProductUOM: true },
              });
              if (productWithUOMs) {
                const altUOM = productWithUOMs.ProductUOM.find(
                  (u) => u.name.toLowerCase() === poItem.uom.toLowerCase()
                );
                if (altUOM) {
                  return item.receivedQuantity * Number(altUOM.conversionFactor);
                }
              }
            }
            // No conversion needed; quantity already in base UOM
          ...
          const batch = await tx.inventoryBatch.create({
            data: {
              ...
              unitCost: Number(unitCostInBaseUOM.toFixed(4)), // Store cost per base unit
              receivedDate,
              expiryDate,
              status: 'active',
            },
