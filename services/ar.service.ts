...
export class ARService {
  ...
  async recordPayment(data: RecordARPaymentInput) {
    return await prisma.$transaction(async (tx) => {
      ...
      if (!ar) {
        throw new NotFoundError('Accounts Receivable', data.arId);
      }

      // Validate payment amount
      if (data.amount <= 0) {
        throw new ValidationError('Payment amount must be greater than 0', { amount: ['Amount must be greater than 0'] });
      }

      if (new Prisma.Decimal(data.amount).greaterThan(ar.balance)) {
        throw new ValidationError('Payment amount exceeds outstanding balance', { amount: ['Payment amount cannot exceed outstanding balance'] });
      }
