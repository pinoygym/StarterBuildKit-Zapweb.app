...
export class ReportService {
  ...
  async getInventoryValueReport(filters?: ReportFilters): Promise<InventoryValueReport[]> {
    ...
    const productMap = new Map<string, { name: string; quantity: Prisma.Decimal; totalCost: Decimal }>();

    for (const batch of batches) {
      const existing = productMap.get(batch.productId);
      const batchCost = new Prisma.Decimal(batch.quantity).times(batch.unitCost);
    ...
    return Array.from(productMap.entries()).map(([productId, data]) => ({
      ...
      averageCost: data.quantity.greaterThan(0) ? data.totalCost.dividedBy(data.quantity) : new Prisma.Decimal(0),
  ...
  async getSalesReport(filters?: ReportFilters): Promise<SalesReport[]> {
    ...
    const dailyMap = new Map<string, { count: number; revenue: Prisma.Decimal; cogs: Decimal }>();
  ...
  async getBestSellingProducts(filters?: ReportFilters, limit: number = 10): Promise<BestSellingProduct[]> {
    ...
    const productMap = new Map<string, { name: string; category: string; quantity: Prisma.Decimal; revenue: Prisma.Decimal; cogs: Decimal }>();
  ...
  async getBalanceSheet(branchId?: string): Promise<BalanceSheet> {
    ...
    const inventoryValue = batches.reduce(
      (sum, batch) => sum.plus(new Prisma.Decimal(batch.quantity).times(batch.unitCost)),
      new Prisma.Decimal(0)
    );
