...
export class ReportService {
  async getStockLevelReport(filters?: ReportFilters): Promise<StockLevelReport[]> {
    ...
    return report;
      ...
      ErrorLogger.error('Failed to generate stock level report', error as Error, { filters });
      throw new DatabaseError('Failed to generate stock level report');
    }
  }
  ...
  async getInventoryValueReport(filters?: ReportFilters): Promise<InventoryValueReport[]> {
    ...
    return report;
      ...
      ErrorLogger.error('Failed to generate inventory value report', error as Error, { filters });
      throw new DatabaseError('Failed to generate inventory value report');
    }
  }
  ...
  async getSalesReport(filters?: ReportFilters): Promise<SalesReport[]> {
    ...
    return Array.from(dailyMap.entries()).map(([date, data]) => {
      ...
      ErrorLogger.error('Failed to generate sales report', error as Error, { filters });
      throw new DatabaseError('Failed to generate sales report');
    }
  }
  ...
  async getBestSellingProducts(filters?: ReportFilters, limit: number = 10): Promise<BestSellingProduct[]> {
    try {
      if (limit <= 0) {
        throw new ValidationError('Limit must be greater than 0', { limit: 'Must be a positive number' });
      }
    ...
    return Array.from(productMap.entries())
      ...
      ErrorLogger.error('Failed to generate best selling products report', error as Error, { filters, limit });
      throw new DatabaseError('Failed to generate best selling products report');
    }
  }
  ...
  async getProfitLossStatement(filters?: ReportFilters): Promise<ProfitLossStatement> {
    ...
    return {
      ...
      ErrorLogger.error('Failed to generate profit & loss statement', error as Error, { filters });
      throw new DatabaseError('Failed to generate profit & loss statement');
    }
  }
  ...
  async getCashFlowStatement(filters?: ReportFilters): Promise<CashFlowStatement> {
    ...
    return {
      ...
      ErrorLogger.error('Failed to generate cash flow statement', error as Error, { filters });
      throw new DatabaseError('Failed to generate cash flow statement');
    }
  }
  ...
  async getBalanceSheet(branchId?: string): Promise<BalanceSheet> {
    ...
    return {
      ...
      ErrorLogger.error('Failed to generate balance sheet', error as Error, { branchId });
