...
export const receivingVoucherItemSchema = z.object({
  productId: idSchema,
  poItemId: idSchema, // Required to match CreateReceivingVoucherItemInput type
...
export const createReceivingVoucherSchema = z.object({
  ...
  supplierDiscount: z.number().min(0, 'Discount cannot be negative').optional(),
  supplierDiscountType: z.enum(['percentage', 'fixed']).optional(),
  additionalFees: z.number().min(0, 'Additional fees cannot be negative').optional(),
  additionalFeesDescription: z.string().max(200, 'Description too long').optional(),
  recomputeAverageCost: z.boolean().optional(),
}).refine(
  (data) => {
    // If discount type is percentage, ensure discount is <= 100
    if (data.supplierDiscountType === 'percentage' && data.supplierDiscount) {
      return data.supplierDiscount <= 100;
    }
    return true;
  },
  {
...
export const cancelReceivingVoucherSchema = z.object({
...
export type CancelReceivingVoucherInput = z.infer<typeof cancelReceivingVoucherSchema>;
